/*
 * phase4_camera.c — Interactive Camera
 *
 * Orbit camera with WASD/arrow controls, zoom, smooth interpolation.
 * Integrates terminal framework and SDF renderer.
 *
 * Deliverable: Fly around the scene interactively, rotate/zoom around objects.
 *
 * Build: gcc -O3 -o phase4_camera phase4_camera.c -lm
 * Run:   ./phase4_camera
...
static const uint8_t MATERIAL_COLORS[][3] = {
    {1, 2, 1}, {5, 1, 1}, {1, 3, 5}, {5, 4, 0}, {3, 3, 3},
    {5, 5, 0}, {5, 0, 5}, {0, 5, 5}, {5, 2, 0}, {2, 5, 2},
};
...
static ShadeResult shade_point(vec3 p, vec3 rd, vec3 normal, int mat_id, float dist) {
    ...
    if (shadow.hit && shadow.dist < 15.0f) {
        float shadow_factor = 0.3f + 0.7f * (shadow.dist / 15.0f);
...
static void draw_hud(OrbitCamera *cam, double fps) {
    ...
    snprintf(hud, sizeof(hud),
        ...
        " MujoCol Phase 4 | FPS: %.0f | Cam: (%.1f, %.1f, %.1f) | [WASD/Arrows] rotate [+/-] zoom [Q] quit ",
    ...
    while (running) {
        ...
        while ((key = read_key()) != KEY_NONE) {
            ...
            switch (key) {
                ...
                case 'w': case 'W': case KEY_UP:   cam.elevation += move_speed; break;
                case 's': case 'S': case KEY_DOWN: cam.elevation -= move_speed; break;
                case 'a': case 'A': case KEY_LEFT: cam.azimuth -= move_speed; break;
                case 'd': case 'D': case KEY_RIGHT: cam.azimuth += move_speed; break;
        ...
        /* Frame rate limiting - aim for ~10 FPS to balance responsiveness and computation */
        sleep_ms(50);
    }
